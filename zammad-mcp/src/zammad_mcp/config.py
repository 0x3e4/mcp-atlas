"""Configuration for the Zammad MCP server, loaded from environment variables.

Authentication is a Zammad API token (Profile → Token Access; the token's permissions govern what
it can read/write). Secrets are never hardcoded; everything comes from the process environment.
"""
...
_REQUIRED = ("ZAMMAD_BASE_URL", "ZAMMAD_TOKEN")
    ...
    def from_env(cls, env: dict[str, str] | None = None) -> "Settings":
        ...
        if missing:
            raise ConfigError(
                ...
                + ", ".join(missing)
                + ". Set them (e.g. via --env-file ./zammad.env) before starting the server."
            )
        ...
        return cls(
            ...
            timeout=_float_env(env, "ZAMMAD_TIMEOUT", 30.0),
            max_rows=_int_env(env, "ZAMMAD_MAX_ROWS", 50),
            verify_ssl=_bool_env(env, "ZAMMAD_VERIFY_SSL", True),
            ca_bundle=env.get("ZAMMAD_CA_BUNDLE", "").strip(),
        )

    @property
    def base_origin(self) -> str:
        """Scheme + host of the base URL (e.g. ``https://zammad.example.com``)."""
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}"

    @property
    def api_base(self) -> str:
        """Base path for Zammad REST API requests (``<base_url>/api/v1``)."""
        return f"{self.base_url}/api/v1"

    @property
    def auth_headers(self) -> dict[str, str]:
        """Zammad token auth header."""
