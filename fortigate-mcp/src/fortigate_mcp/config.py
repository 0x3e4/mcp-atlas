...
_REQUIRED = ("FORTIGATE_BASE_URL", "FORTIGATE_API_TOKEN")
    ...
    def from_env(cls, env: dict[str, str] | None = None) -> "Settings":
        ...
        if missing:
            raise ConfigError(
                ...
                + ", ".join(missing)
                + ". Set them (e.g. via --env-file ./fortigate.env) before starting the server."
            )
        ...
        return cls(
            ...
            timeout=_float_env(env, "FORTIGATE_TIMEOUT", 30.0),
            max_rows=_int_env(env, "FORTIGATE_MAX_ROWS", 200),
            verify_ssl=_bool_env(env, "FORTIGATE_VERIFY_SSL", True),
            ca_bundle=env.get("FORTIGATE_CA_BUNDLE", "").strip(),
        )

    @property
    def base_origin(self) -> str:
        """Scheme + host of the base URL (e.g. ``https://192.0.2.1``)."""
        parts = urlsplit(self.base_url)
        return f"{parts.scheme}://{parts.netloc}"

    @property
    def api_base(self) -> str:
        """Base path for FortiOS REST API v2 requests (``<base_url>/api/v2``)."""
